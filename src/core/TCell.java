...
public class TCell extends MigratoryCell implements AttractantSecretor, ChemotaxisResponsive
{
	...
	public static double chemotaxisConcentrationLocation;
	public static double chemotaxisConcentrationShape;
	private static LogNormalDistribution chemokineSensitivityDistro;
	private double chemokineConcentrationResponseThreshold = Double.POSITIVE_INFINITY;  // Cell-specific sensitivity. 
	...
	public TCell(Schedule sched)
	{
		...
		if (secretionMode != SecretionModeTypes.None)
			chemokineConcentrationResponseThreshold = chemokineSensitivityDistro.sample();
	}
	...
	public void step(SimState state)
	{
		...
		if (this.motile)
		{
			...
			if (bounce.lengthSq() != 0.0)
			{
				...
				// Handles case for NON bootstrapped chemotaxis.
				if (! (   secretionMode == SecretionModeTypes.None
	...
	public static void loadParameters(Document params) throws XPathExpressionException
	{
		...
		if (secretionMode != SecretionModeTypes.None)
		{
			...
			chemokineSensitivityDistro = new LogNormalDistribution(Math.log(chemotaxisConcentrationLocation),
